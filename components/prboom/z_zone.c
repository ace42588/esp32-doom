...
#else
  // Try to allocate with ESP32-specific memory capabilities
  block = heap_caps_malloc(size + HEADER_SIZE, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);
  if (!block) {
    // Try PSRAM only as fallback
